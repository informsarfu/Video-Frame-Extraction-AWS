...
class aws_grader():
    ...
    def count_images(self, TC_num):
        bucket = self.s3_resources.Bucket(self.out_bucket_name)
        image_formats = ('.jpg', '.jpeg', '.png', '.gif')
        image_count = {}

        for obj in bucket.objects.all():
            # Check if the object key ends with an image format
            if obj.key.lower().endswith(image_formats):
                folder_name = obj.key.split('/')[0]  # Assuming folder structure in keys
